...
export async function POST(request: Request) {
    ...
    const transcript_update = await xata.db.info.update(info_record[0].id,{transcript_url: transcript_url, sentence_url:sentence_url, transcription_status: "completed"});
    return NextResponse.json({ message: "Hello, world!" })
}